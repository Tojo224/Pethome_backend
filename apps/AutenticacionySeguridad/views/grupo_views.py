...
class GrupoUsuarioDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        _registrar_bitacora_seguro(
            BitacoraService.registrar_evento,
            accion=BitacoraAccion.GRUPO_ELIMINADO,
            descripcion=f"Grupo de usuario '{nombre}' eliminado.",
            usuario=request.user,
            request=request,
            modulo=BitacoraModulo.AUTENTICACION,
            entidad_tipo="GrupoUsuario",
...
class GrupoPermisoComponenteListCreateView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if response.status_code == 201:
            _registrar_bitacora_seguro(
                BitacoraService.registrar_evento,
                accion=BitacoraAccion.PERMISO_ASIGNADO,
                descripcion=f"Permiso asignado al grupo ID {response.data.get('grupo')} para el componente {response.data.get('componente_nombre') or response.data.get('componente')}.",
                usuario=request.user,
                request=request,
                modulo=BitacoraModulo.AUTENTICACION,
...
class GrupoPermisoComponenteDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        _registrar_bitacora_seguro(
            BitacoraService.registrar_evento,
            accion=BitacoraAccion.PERMISO_REMOVIDO,
            descripcion=f"Permiso ID {permiso_id} removido del grupo ID {grupo_id}.",
            usuario=request.user,
            request=request,
            modulo=BitacoraModulo.AUTENTICACION,
            entidad_tipo="GrupoPermisoComponente",
